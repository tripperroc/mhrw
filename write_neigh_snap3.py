...
def main():
    ...
    u = set_orientation_by_file (sys.argv[1], sys.argv[8], "orientation", u)
    u = set_orientation_by_file (sys.argv[2], sys.argv[8], "test_orientation", u)
    

    all = u.subgraph([x for x in u.node if "orientation" in u.node[x]])
    gay_graph = u.subgraph([y for y in [x for x in g.node if "orientation" in g.node[x]] if g.node[y]['orientation'] == 1])
